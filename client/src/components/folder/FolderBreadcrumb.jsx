...
export default function FolderBreadcrumb({ breadcrumbs = [] }) {
  return (
    <nav aria-label="Directory navigation" className="flex items-center gap-1.5 text-xs font-mono text-[var(--theme-text-muted)] flex-wrap select-none">
      <Link
        to="/dashboard"
        className="px-2 py-1 rounded-lg border border-[var(--theme-border)] bg-[var(--theme-surface)]/80 text-[var(--theme-accent)] hover:border-[var(--theme-accent)]/40 hover:text-[var(--theme-text)] font-semibold flex items-center gap-1.5 transition-all shadow-sm"
      >
        ...
        <span className="tracking-wider text-[11px]">VAULT</span>
      </Link>
      ...
      {breadcrumbs.map((crumb, idx) => {
        ...
        return (
          <span key={crumb.id || idx} className="flex items-center gap-1.5">
            ...
            {isLast ? (
              <span className="px-2 py-1 rounded-lg bg-[var(--theme-surface)] border border-[var(--theme-accent)]/30 text-[var(--theme-text)] font-semibold flex items-center gap-1.5 shadow-sm">
