...
export default function FolderPage() {
  ...
  const { files, loading: filesLoading, sortBy, sortOrder, page, limit, pagination, setSortBy, setSortOrder, setPage, setLimit, fetchFiles, togglePrivacy, deleteFile, renameFile, updateFileInState } = useFiles(folderId);
  ...
  const handleToggleSelectFolder = useCallback((targetFolderId) => {
    setSelectedFolderIds((prev) => {
      const next = new Set(prev);
      if (next.has(targetFolderId)) next.delete(targetFolderId);
      else next.add(targetFolderId);
      return next;
    });
  }, []);
    ...
    <div className="space-y-6 fade-in select-none pb-20">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-[var(--theme-border)]">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-[var(--theme-text)] mt-2">
            {currentFolderName}
          </h1>
        </div>
